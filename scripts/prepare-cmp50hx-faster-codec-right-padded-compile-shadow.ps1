param(
    ...
    [string]$DestinationPath = 'tmp\cmp50hx-faster-codec-right-padded-compile-shadow'
)
...
$patches = @((Join-Path $PSScriptRoot 'patches\cmp50hx-faster-codec-right-padded-compile.patch'))
foreach ($patch in $patches) {
    if (-not (Test-Path -LiteralPath $patch -PathType Leaf)) {
        throw "Required codec candidate patch was not found: $patch"
    }
}
...
try {
    foreach ($patch in $patches) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Codec candidate patch check failed: $patch (exit=$LASTEXITCODE)"
        }
        & git apply "--directory=$destinationRelative" $patch
        if ($LASTEXITCODE -ne 0) {
            throw "Codec candidate patch apply failed: $patch (exit=$LASTEXITCODE)"
        }
    }
    $candidateModel = Join-Path $destination 'faster_qwen3_tts\model.py'
    foreach ($marker in @('def _decode_right_padded_window(', 'def _enable_right_padded_decoder_compile(')) {
        if (-not (Select-String -LiteralPath $candidateModel -SimpleMatch $marker -Quiet)) {
            throw "Codec candidate patch did not add its expected marker: $marker"
        }
    }
}
catch {
