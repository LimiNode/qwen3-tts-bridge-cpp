"""Fail closed when evaluating post-repair corpus-v4 targeted human review."""
...
try:
    from scripts.prepare_corpus_v4_targeted_rereview import _SCHEMA_VERSION, _prepare_rows
except ModuleNotFoundError:
    from prepare_corpus_v4_targeted_rereview import _SCHEMA_VERSION, _prepare_rows
...
def main() -> int:
    ...
    expected = _prepare_rows(args.repair_authoring.read_bytes(), args.candidate.read_bytes())
    summary = _evaluate(_load_jsonl(args.input), expected)
    _write_object(args.output, summary)
    print(json.dumps(summary, sort_keys=True))
    return 0 if summary["passed"] else 1


def _evaluate(review: list[dict[str, Any]], expected: list[dict[str, object]]) -> dict[str, object]:
    expected_by_id = _by_id(expected, "expected review")
    review_by_id = _by_id(review, "review")
    if set(review_by_id) != set(expected_by_id):
        raise RuntimeError("review record IDs do not match the repair authoring")
    ...
    checks = Counter(reason for values in failures.values() for reason in values)
    return {
        "targeted_rereview_summary_schema_version": 1,
        "review_scope": "post_repair_corpus_v4_targeted_rereview",
        "review_record_count": len(review),
        "reviewer_id": next(iter(reviewers), ""),
        "repair_authoring_sha256": expected[0]["repair_authoring_sha256"],
        "candidate_sha256": expected[0]["candidate_sha256"],
...
def _record_failures(row: dict[str, Any], template: dict[str, object]) -> list[str]:
    failures = []
    if set(row) != set(template):
        return ["schema"]
    if any(row[field] != template[field] for field in set(template).difference(_MUTABLE_FIELDS)):
        failures.append("provenance_or_content_mismatch")
    if row["targeted_rereview_schema_version"] != _SCHEMA_VERSION:
