"""Fail closed when evaluating final-text corpus-v4 targeted human review."""
...
try:
    from scripts.prepare_corpus_v4_final_targeted_review import (
        _SCHEMA_VERSION,
        _prepare_rows,
    )
except ModuleNotFoundError:
    from prepare_corpus_v4_final_targeted_review import _SCHEMA_VERSION, _prepare_rows
...
def main() -> int:
    ...
    expected = _prepare_rows(args.targeted_form.read_bytes(), args.candidate.read_bytes())
...
def _record_failures(row: dict[str, Any], template: dict[str, object]) -> list[str]:
    ...
    if row["final_targeted_review_schema_version"] != _SCHEMA_VERSION:
