"""Prepare human authoring for failed records from a corpus-v4 manual review."""
...
def _prepare_rows(
    ...
    candidate_by_id = _by_key(candidate_rows, "record_id", "candidate")
    review_by_id = _by_key(review_rows, "label", "review")
    failures = _failure_fields(review_rows)
    if not failures:
        raise RuntimeError("review contains no failed records")
    if summary.get("passed") is not False:
        raise RuntimeError("review summary does not record a failed review")
    ...
    for record_id in sorted(failures):
        ...
        if review.get("text") != record.get("text"):
            raise RuntimeError(f"{record_id}: review text no longer matches candidate")
...
def _failure_fields(review_rows: list[dict[str, Any]]) -> dict[str, list[str]]:
    positive = (
        "category_fidelity",
        "naturalness",
        "likely_real_usage",
    ...
    for row in review_rows:
        ...
        if not isinstance(record_id, str) or not record_id:
            raise RuntimeError("review has an invalid label")
        fields = [field for field in positive if row.get(field) is False]
        if row.get("language_class") == "mixed" and row.get("code_switch_naturalness") is False:
            fields.append("code_switch_naturalness")
        if row.get("generic_experiment_phrasing") is True:
            fields.append("generic_experiment_phrasing")
        if fields:
            failures[record_id] = fields
    return failures
...
def _by_key(
    ...
    for row in rows:
        ...
        if not isinstance(value, str) or not value:
            raise RuntimeError(f"{name} has an invalid {key}")
        if value in result:
            raise RuntimeError(f"{name} has duplicate {key}: {value}")
        result[value] = row
    return result
