param(
    ...
    [string]$DestinationPath = 'tmp\cmp50hx-faster-codec-streaming-shadow'
)
...
if ($sourceHash -ne $expectedModelSha256) {
    throw (
        'Frozen Faster shadow model.py fingerprint is unsupported for this ' +
        "experiment: expected $expectedModelSha256, got $sourceHash"
    )
}

$patch = Join-Path $PSScriptRoot 'patches\cmp50hx-faster-codec-streaming-decode.patch'
if (-not (Test-Path -LiteralPath $patch -PathType Leaf)) {
    throw "Codec streaming patch was not found: $patch"
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Codec streaming patch check failed (exit=$LASTEXITCODE)"
    }
    & git apply "--directory=$destinationRelative" $patch
    if ($LASTEXITCODE -ne 0) {
        throw "Codec streaming patch apply failed (exit=$LASTEXITCODE)"
    }
    ...
    if (-not (Select-String -LiteralPath $candidateModel -SimpleMatch `
        ...
        throw 'Codec streaming patch did not add its expected marker'
    }
}
catch {
    if (Test-Path -LiteralPath $destination) {
...
$candidateModel = Join-Path $destination 'faster_qwen3_tts\model.py'
$candidateHash = (Get-FileHash -LiteralPath $candidateModel -Algorithm SHA256).Hash
Write-Output "candidate_shadow=$destination"
Write-Output "candidate_model_sha256=$candidateHash"
