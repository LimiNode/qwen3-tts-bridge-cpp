...
runtime can import `qwen_tts.inference.qwen3_tts_model`. It does not load model
weights or run inference, so real audio validation still belongs on a machine
with the target GPU and local model files.
