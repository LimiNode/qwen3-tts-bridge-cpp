...
`completion_execution_outcome`. Native runs may provide explicit `natural_eos`
or `max_tokens` without a model generation trace; Python/Faster runs must
provide a generation trace proving natural EOS. A bare `completed` without
