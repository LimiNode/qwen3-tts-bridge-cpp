...
- Это модель preset-голосов. Клонирование голоса ещё не доступно в публичном
  сценарии bridge.
- Текущий worker намеренно не передаёт `instruction` в generation path модели
  `0.6B` CustomVoice. `/style` принимается CLI, но не делает речь агрессивной,
  шёпотом, быстрее или иначе стилистически отличающейся.
